...
orthogonal({{X1,Y1},{X2,Y2}},{{A1,B1},{A2,B2}}) ->
    TX = X1-X2,
    TY = Y1-Y2,
    TA = A1-A2,
    TB = B1-B2,
...
parse(S1,S2) ->
    ...
    K=findmin(lists:append(L1,L2),{0,0}),
    [F1|F1S]=L1, %%transpose(L1,K),
    [F2|F2S]=L2, %%transpose(L2,K),
    {feppelize(F1S,F1,[]),feppelize(F2S,F2,[])}.
...
findcoll(S1,S2) ->  
    ...
    D = lists:map(fun(D)->intersect(D) end,C),
    E = lists:map(fun({X,Y})->{X,Y,abs(X)+abs(Y)} end,D),
