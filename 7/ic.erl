...
r2(2,[A,B,C|L],O,Pid) ->
    ...
    P = setnth(C+1,O,X),
    r(lists:nthtail(length(P)-length(L),P),P,Pid);
r2(3,[A|L],O,Pid) ->
...
r2(4,[A|L],O,Pid) ->
    ...
    %io:fwrite("send ~B\n",[X]),
    Pid ! X,
    r(lists:nthtail(length(O)-length(L),O),O,Pid);
r2(5,[A,B|L],O,Pid) ->
    X = lists:nth(A+1,O),
    Y = lists:nth(B+1,O),
