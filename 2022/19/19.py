...
front=SortedSet(key=lambda x:-x[3]*(x[2][1]*2+x[2][2]*4+x[2][3]*8))
...
#    print(inv)
    ...
    if inv[ore]>=bp[geode][0] and inv[obsidian]>=bp[geode][1]:
        ...
        front.add((bp, ap, (inv[ore]-bp[geode][0], inv[clay], inv[obsidian]-bp[geode][1], inv[geode]), t+1,tl+mkstr(t,inv,bots,"geode")))
#        print("bpt")
        
    if inv[ore]>=bp[obsidian][0] and inv[clay]>=bp[obsidian][1]:
        ap = (bots[0],bots[1],bots[2]+1,bots[3])
        front.add((bp, ap, (inv[ore]-bp[obsidian][0],inv[clay]-bp[obsidian][1],inv[obsidian],inv[geode]),t+1,tl+mkstr(t,inv,bots,"obs")))
        #        print("bpt2")

    if inv[ore]>=bp[clay]:
        ap = (bots[0],bots[1]+1,bots[2],bots[3])
        front.add((bp, ap, (inv[ore]-bp[clay],inv[clay],inv[obsidian],inv[geode]),t+1,tl+mkstr(t,inv,bots,"clay")))
#       print("bpt3")
        
    if inv[ore]>=bp[ore]:
        ap = (bots[0]+1,bots[1],bots[2],bots[3])
        front.add((bp, ap, (inv[ore]-bp[ore],inv[clay],inv[obsidian],inv[geode]),t+1,tl+mkstr(t,inv,bots,"ore")))
#        print("bpt4")
        
    front.add((bp, tuple(bots), tuple(inv),t+1,tl+mkstr(t,inv,bots,"")))
#    print("bpt5")
...
while len(front):
    v = front.pop()
    (bp,bots,inv,t,tl) = v
    diggy(bp,bots,inv,t,tl)
    if not len(front) % 10000:
